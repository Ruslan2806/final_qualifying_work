...
def plot_telemetry():
    try:
        with open(str(TELEMETRY_LOG_PATH), "r") as f:
            data = json.load(f)
    ...
    ax1.plot(t, z_raw, 'r--', alpha=0.5, label='\"cырые\" данные')
    ...
    ax2.plot(t, np.array(v_raw) * 3.6, 'r--', alpha=0.5, label='\"сырая\" скорость')
    ax2.plot(t, np.array(v_filt) * 3.6, 'g-', linewidth=2, label='скорость (Калман и EMA)')
