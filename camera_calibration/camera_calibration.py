...
from PyQt5.QtWidgets import (
    ...
    QGroupBox, QScrollArea, QMessageBox, QFileDialog, QGridLayout
)
...
class ImageLabel(QLabel):
    ...
    def wheelEvent(self, event):
        if not self._pixmap_src: return
        ...
        if sa:
            ...
            sa.horizontalScrollBar().setValue(int(sa.horizontalScrollBar().value() + delta.x()))
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.cont_layout = QGridLayout(self.container)
        self.cont_layout.addWidget(self.img_label, 0, 0, Qt.AlignCenter)
        ...
        self.lbl_sel = QLabel("Не выбрана"); self.lbl_sel.setStyleSheet("color: #ffa500; font-weight: bold;")
        add_layout.addWidget(self.lbl_sel)
        row = QHBoxLayout(); row.addWidget(QLabel("D (м):"))
        self.input_dist = QLineEdit(); self.input_dist.returnPressed.connect(self.add_point)
        row.addWidget(self.input_dist); add_layout.addLayout(row)
        self.btn_add = QPushButton("Добавить"); self.btn_add.setEnabled(False); self.btn_add.clicked.connect(self.add_point)
        ...
        self.lbl_pts = QLabel("Пусто"); self.lbl_pts.setWordWrap(True)
        pts_layout.addWidget(self.lbl_pts)
        self.btn_undo = QPushButton("Удалить последнюю"); self.btn_undo.clicked.connect(self.undo_point)
    ...
    def on_point_selected(self, y_orig):
        self.lbl_sel.setText(f"Y: {y_orig}px"); self.btn_add.setEnabled(True); self.input_dist.setFocus()
    ...
    def add_point(self):
        ...
        self.input_dist.clear(); self.btn_add.setEnabled(False); self._refresh_ui()

    def undo_point(self):
        if self.points: self.points.pop(); self._refresh_ui()
    ...
    def _btn_style(color):
        return f"QPushButton {{ background-color: {color}; color: white; border-radius: 5px; font-weight: bold; border: none; padding: 5px; }} QPushButton:hover {{ opacity: 0.8; }} QPushButton:disabled {{ background-color: #333; color: #666; }}"

    @staticmethod
    def _grp_style():
        return "QGroupBox { font-weight: bold; color: #bbb; border: 1px solid #444; margin-top: 10px; padding-top: 10px; border-radius: 5px; } QGroupBox::title { subcontrol-origin: margin; left: 10px; }"
...
if __name__ == "__main__":
    ...
    p = QPalette()
    p.setColor(QPalette.Window, QColor(45, 45, 45)); p.setColor(QPalette.WindowText, Qt.white)
    p.setColor(QPalette.Base, QColor(30, 30, 30)); p.setColor(QPalette.Text, Qt.white)
    app.setPalette(p)
    win = MainWindow(); win.resize(1300, 900); win.show()
    sys.exit(app.exec_())